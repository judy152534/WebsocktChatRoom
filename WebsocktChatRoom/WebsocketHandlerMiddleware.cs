        ...
        private async Task MessageRoute(Message message)
        {
            ...
            switch (message.action)
            {
                case "join":
                    ...
                    #region 如果有已經存在的nickName，則踢掉前一個相同名字的WS連接
                    var clientList = WebsocketClientCollection.GetRoomClients(client.RoomNo);
                    var duplicate = clientList.SingleOrDefault(p => p.NickName == message.nick);
                    if (duplicate != null) 
                    {
                        await duplicate.SendMessageAsync($"You are disconnected now. Other user with same nickName is now join!");
                        await duplicate.WebSocket.CloseAsync(closeStatus: WebSocketCloseStatus.NormalClosure,
                                     statusDescription: "Closed by the ConnectionManager",
                                     cancellationToken: CancellationToken.None);
                        clientList.Remove(duplicate);
                    }
